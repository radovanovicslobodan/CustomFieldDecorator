...
public class PrimeVueTest {
  ...
  public void dummyTest() throws InterruptedException {
    ...
    driver.get("https://v3.primevue.org/datatable/#virtualscroll");
    List<WebElement> rowsInitial = driver.findElements(
        By.cssSelector("section:nth-child(17) tbody tr"));
    var lastRowInitial = rowsInitial.get(rowsInitial.size() - 1).getAttribute("data-p-index");

    WebElement scrollableElement = driver.findElement(
        By.cssSelector("section:nth-child(17) .p-virtualscroller"));
    js.executeScript("arguments[0].scrollTop += 230;", scrollableElement);

    List<WebElement> rowsAfterScrolling = driver.findElements(
        By.cssSelector("section:nth-child(17) tbody tr"));
    var lastRowAfterScrolling = rowsAfterScrolling.get(rowsAfterScrolling.size() - 1)
        .getAttribute("data-p-index");
    Thread.sleep(2000);
  ...
  public void findRowTestBlueprint() {
    ...
    WebElement firstElement = driver.findElement(
        By.cssSelector(
            "section:nth-child(17) tbody tr[0]")); // fix finding first element if its not correct
  ...
  public void findRowTest() {
    ...
    String targetId = "1000"; // 👈 SET YOUR TARGET ID HERE

    Set<Integer> seenIndices = new HashSet<>();
    int scrollStep = itemHeight * 25; // scroll by ~25 rows
    int initialScroll = itemHeight * 7;
    int maxAttempts = 51; // safety limit
    int attempts = 0;

    List<Row> mappedRows = List.of();
...
//      seenIndices.add(currentMaxIndex);
      ...
      try {
        wait.until(d -> {
          List<WebElement> newRows = d.findElements(
              By.cssSelector("section:nth-child(17) tbody tr"));
    ...
    System.out.println(mappedRows.get(mappedRows.size() - 1).text);
